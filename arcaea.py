...
@sv.on_prefix(['arcscore', 'Arcscore', 'ARCSCORE', 'arcsc', 'Arcsc', 'ARCSC', 'arcbest', 'Arcbest', 'ARCBEST', 'Score', 'score', 'SCORE'])
...
async def bind(bot: NoneBot, ev: CQEvent):
    ...
    try:
        ...
        msg = f'用户 {data}({arcid}) 已成功绑定QQ {qqid}，现可使用 <arcinfo> 指令查询B30成绩和 <arcre> 指令查询最近游玩'
        await bot.send(ev, msg, at_sender=True)
    except ActionFailed:
        msg = f'用户 {arcid} 已成功绑定QQ {qqid}，现可使用 <arcinfo> 指令查询B30成绩和 <arcre> 指令查询最近游玩'
        await bot.send(ev, msg, at_sender=True)
    except ArcError as e:
        await bot.send(ev, f'Error：{e.error_str}', at_sender=True)
